...
from ai import STATUS_LABELS

def get_site_name(url: str) -> str:
    """Extracts the main site name to display in the visual tag."""
    host = urlparse(url).netloc.replace("www.", "")
    return host.upper()

def clean_title(title: str) -> str:
    """Removes news site names from the end of article titles."""
    suffixes = [
...
def group_relevant_items(relevant_items: list) -> list:
    """Groups articles by exam based on the 'group' tag generated by the AI."""
    groups = {}
    ...
    # Sorts so that exams with more articles appear first
    group_list.sort(key=lambda g: g["size"], reverse=True)
    return group_list

def render_group_card(idx: int, group: dict) -> str:
    """Generates the inner HTML for an exam card, including carousel when needed."""
    items = group["items"]
    size = group["size"]
...
def generate_html(groups: list, date_str: str, total_analyzed: int, total_relevant: int) -> str:
    """Generates the full HTML page to be hosted on GitHub Pages."""
